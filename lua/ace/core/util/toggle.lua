...
function M.wrap(toggle)
	return setmetatable(toggle, {
		__call = function()
			...
			if state then
				Ace.info("Enabled " .. toggle.name, { title = toggle.name })
			else
				Ace.warn("Disabled " .. toggle.name, { title = toggle.name })
			end
			return state
		end,
	}) --[[@as ace.Toggle.wrap]]
...
function M.wk(lhs, toggle)
	if not Ace.has("which-key.nvim") then
		return
	end
	local function safe_get()
		local ok, enabled = pcall(toggle.get)
		if not ok then
			Ace.error("Failed to get toggle state for **" .. toggle.name .. "**:\n", { once = true })
		end
		return enabled
	end
...
function M.format(buf)
	return M.wrap({
		...
		get = function()
			...
			return Ace.format.enabled()
		end,
		set = function(state)
			Ace.format.enable(state, buf)
		end,
	})
end
