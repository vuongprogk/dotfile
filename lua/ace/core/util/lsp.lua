...
function M.rename_file()
	...
	local old = assert(Ace.root.realpath(vim.api.nvim_buf_get_name(buf)))
	local root = assert(Ace.root.realpath(Ace.root.get({ normalize = true })))
...
function M.formatter(opts)
	...
	local ret = {
		...
		priority = 1,
		format = function(buf)
			M.format(Ace.merge({}, filter, { bufnr = buf }))
		end,
		sources = function(buf)
			local clients = M.get_clients(Ace.merge({}, filter, { bufnr = buf }))
			---@param client vim.lsp.Client
	...
	return Ace.merge(ret, opts) --[[@as Formatter]]
end
...
function M.format(opts)
	opts = vim.tbl_deep_extend(
		...
		{},
		opts or {},
		Ace.opts("nvim-lspconfig").format or {},
		Ace.opts("conform.nvim").format or {}
	)
	...
	M.on_supports_method("textDocument/documentHighlight", function(_, buf)
		vim.api.nvim_create_autocmd({ "CursorHold", "CursorHoldI", "CursorMoved", "CursorMovedI" }, {
			...
			callback = function(ev)
				if not require("ace.coding.lsp.keymaps").has(buf, "documentHighlight") then
					return false
				end
				...
				if not ({ M.words.get() })[2] then
					if ev.event:find("CursorMoved") then
						vim.lsp.buf.clear_references()
					elseif not Ace.cmp.visible() then
