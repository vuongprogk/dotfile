...
function M.cmp_source(name, icon)
	...
	local colors = {
		ok = Ace.ui.fg("Special"),
		error = Ace.ui.fg("DiagnosticError"),
		pending = Ace.ui.fg("DiagnosticWarn"),
	}

	return {
		function()
			return icon or Ace.config.icons.kinds[name:sub(1, 1):upper() .. name:sub(2)]
		end,
		cond = function()
			return status() ~= nil
		end,
...
function M.pretty_path(opts)
	...
	return function(self)
		...
		local root = Ace.root.get({ normalize = true })
		local cwd = Ace.root.cwd()
